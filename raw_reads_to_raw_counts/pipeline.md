...
### Notes
- Each sample’s microbiome ORF count table (two columns: `orf_id` and `<sample>_counts`) produced by `make_microbiome_counts.py` was merged into one study-level matrix in R.  
- A full outer join on `orf_id` was used so the final table contained the union of all ORFs detected across samples.  
- Cells with missing values after merging were filled with `0`, creating a sparse matrix of **ORFs × samples** with raw read counts.  
- Column names were standardized to match sample identifiers in the metadata before export.  
- For downstream analyses, read counts were normalized to **RPKM (Reads Per Kilobase per Million mapped reads)** in R to account for library size and ORF length.  
- The count tables uploaded to this GitHub repository contain the **raw, unnormalized counts**, allowing users to apply their own preferred normalization methods.
...
- Transcript-level abundance files (`quant.sf`) generated by **Salmon** for each oyster sample were imported and merged in R using the **tximport** package (Bioconductor).  
- `tximport` automatically read all per-sample output directories and combined them into a single count matrix using transcript identifiers from the *C. virginica* reference transcriptome.  
- The resulting matrix contains **transcripts × samples**, with raw counts as values.  
- Column names were standardized to match the sample identifiers in the metadata before export.  
- For downstream analyses, transcript counts were normalized to **RPKM (Reads Per Kilobase per Million mapped reads)** in R to account for both transcript length and library size.  
- The count tables uploaded to this GitHub repository contain the **raw, unnormalized counts**, allowing users to apply their own normalization or transformation methods as needed.
